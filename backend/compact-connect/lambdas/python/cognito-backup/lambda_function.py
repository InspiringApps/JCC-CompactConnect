...
This function exports all users from a specified Cognito user pool to S3,
storing each user as a separate JSON file with the object key based on the username.
The exported data is designed to support disaster recovery scenarios.

The function expects an event with:
- user_pool_id: The ID of the Cognito user pool to export
- backup_bucket_name: The name of the S3 bucket to store exports
- user_pool_type: The type of user pool (e.g., 'staff', 'provider') for organization
"""
