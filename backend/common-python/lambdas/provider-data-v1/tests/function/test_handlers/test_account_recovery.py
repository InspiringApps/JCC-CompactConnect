# Copied from backend/compact-connect/lambdas/python/provider-data-v1/tests/function/test_handlers/test_account_recovery.py
...
@mock_aws
@patch('common_lambdas.config._Config.current_standard_datetime', datetime.fromisoformat(MOCK_DATETIME_STRING))
...
class TestInitiateAccountRecovery(TstFunction):
    def setUp(self):
        super().setUp()
        patch('common_lambdas.utils.delayed_function', mock_delay_decorator).start()
    ...
    def test_initiate_account_recovery_returns_generic_200_if_rate_limited(self):
        ...
        with patch('common_lambdas.config._Config.current_standard_datetime', new_callable=PropertyMock) as mock_now:
    ...
    def test_initiate_account_recovery_sends_email_if_authenticated(self):
        from common_lambdas.utils import verify_password
        from handlers.account_recovery import initiate_account_recovery
...
class TestVerifyAccountRecovery(TstFunction):
    ...
    def _when_recovery_token_on_provider_record(self):
        from common_lambdas.utils import hash_password
