# Copied from backend/compact-connect/lambdas/python/purchases/tests/unit/test_purchase_client.py
...
from common_lambdas.exceptions import CCFailedTransactionException, CCInternalException, CCInvalidRequestException
...
def _generate_aslp_compact_configuration(include_licensee_charges: bool = False):
    from common_lambdas.data_model.schema.compact.jcc_compact import Compact
...
def _generate_selected_jurisdictions(jurisdiction_items: list[dict] = None):
    from common_lambdas.data_model.schema.jurisdiction.jcc_jurisdiction import Jurisdiction
...
@mock_aws
@patch('common_lambdas.config._Config.current_standard_datetime', datetime.fromisoformat(MOCK_CURRENT_DATETIME))
