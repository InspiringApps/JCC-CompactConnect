...
class TestNotificationStack(TstAppABC, TestCase):
    ...
    def test_privilege_purchase_notification_not_wired(self):
        notification_stack = self.app.sandbox_backend_stage.notification_stack
        self.assertFalse(hasattr(notification_stack, 'privilege_purchase_processor'))
        self.assertFalse(hasattr(notification_stack, 'privilege_purchase_rule'))

    def test_home_jurisdiction_change_notification_listener_resources_created(self):
        notification_stack = self.app.sandbox_backend_stage.notification_stack
        notification_template = Template.from_stack(notification_stack)
        handler_logical_id = notification_stack.get_logical_id(
            notification_stack.event_processors[
                'HomeJurisdictionChangeNotificationListener'
            ].queue_processor.process_function.node.default_child
        )
        handler = TestNotificationStack.get_resource_properties_by_logical_id(
            handler_logical_id,
            resources=notification_template.find_resources(CfnFunction.CFN_RESOURCE_TYPE_NAME),
        )
        self.assertEqual(
            'handlers.home_jurisdiction_events.home_jurisdiction_change_notification_listener',
            handler['Handler'],
        )
